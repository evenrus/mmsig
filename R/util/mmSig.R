...
plot_signatures = function(sig){
        ...
        Can be called from within the main mm_fit_signatures function or independently
        "
        par(mfrow=c(2,1))
        barplot(as.matrix(t(sig[,-c(ncol(sig)-1, ncol(sig))])), 
        ...
        legend("topright",legend=sort(colnames(sig)[-c(ncol(sig)-1, ncol(sig))]),bty="n", pch=15, 
               col=c(RColorBrewer::brewer.pal(8, "Dark2"), RColorBrewer::brewer.pal(4, "Set3")),
...
mm_fit_signatures = function(muts.input="../data/example_muts_96.txt", 
        ...
        return.fig: TRUE = Generate plot for relative mutational signature contributions
        "
    ...
    if(is.data.frame(sig.input)){
        ...
        consigts.defn <- read.delim(sig.input, stringsAsFactors = F, header=T, row.names=1) 
    } else{
    ...
    if (is.null(sample.sigt.profs)) {
       ...
       mm.sigts <- sort(c("Signature.Subs.01","Signature.Subs.02","Signature.Subs.05","Signature.Subs.08","Signature.Subs.09",
                          "Signature.Subs.13","Signature.Subs.17b","Signature.Subs.18","MM1"))
    ...
    if(return.fig){ # Generate plot for relative mutational signature contributions if return.fig artument is TRUE
        plot_signatures(sig)
    }
